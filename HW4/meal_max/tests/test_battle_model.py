...
def test_prep_combatant(battle_model, sample_meal1):
    ...
    assert battle_model.combatants[0].meal == 'Dumplings'

def test_adding_non_meal_to_combatant(battle_model):
    """Test error when adding a non Meal type to the combatant."""
    with pytest.raises(TypeError, match="combatant_data is not a valid Meal"):
...
def test_check_if_empty_non_empty_playlist(battle_model, sample_meal1):
...
def test_check_if_empty_empty_combatant_list(battle_model):
